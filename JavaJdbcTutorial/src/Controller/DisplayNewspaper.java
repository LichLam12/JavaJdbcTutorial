...
@WebServlet("/DisplayNewspaper")
public class DisplayNewspaper extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public DisplayNewspaper() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int newspaper = Integer.parseInt(request.getParameter("newspaper"));
        
        if(newspaper==1)
        {
        	RequestDispatcher rd= request.getRequestDispatcher("Newspaper1_Logging.jsp");
    		rd.forward(request, response);
        }
        else if(newspaper==2)
        {
        	RequestDispatcher rd= request.getRequestDispatcher("Newspaper2_Logging.jsp");
    		rd.forward(request, response);
        }
        else if(newspaper==3)
        {
        	RequestDispatcher rd= request.getRequestDispatcher("Newspaper3_Logging.jsp");
    		rd.forward(request, response);
        }
